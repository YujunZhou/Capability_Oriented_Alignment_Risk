...
class BatchRewardManager:
    ...
    def verify(self, data):
        ...
        # For proxy gaming, we need to pass the full reward_model to access original_article
        reward_models = [item.non_tensor_batch.get("reward_model", {}) for item in data]

        # Merge reward_model info into extras for proxy gaming tasks
        for i, (extra, reward_model) in enumerate(zip(extras, reward_models)):
            if data_sources[i] == "proxy_gaming_summarization":
                if extra is None:
                    extra = {}
                # Add original_article from reward_model to extra_info
                if "original_article" in reward_model:
                    extra["original_article"] = reward_model["original_article"]
                extras[i] = extra
